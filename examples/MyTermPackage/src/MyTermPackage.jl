...
# Example
```julia
using ERGM, MyTermPackage, Networks, Random
...
# convention of ERGM's own nodematch). The per-vertex getter
# `get_vertex_attribute(net, attr, v)` returns `nothing` for a vertex (or a
# network) without the attribute and allocates nothing per call, which keeps
# `change_stat` allocation-free on the sampler's hot path.
function _counts(t::ReciprocatedHomophily, net, i::Int, j::Int)
    (is_missing_dyad(net, i, j) || is_missing_dyad(net, j, i)) && return false
    vi = get_vertex_attribute(net, t.attr, i)
    vj = get_vertex_attribute(net, t.attr, j)
...
#    what `supports_missing` claims. `Networks.supports_missing` and
#    `ERGM.supports_missing` are one function (`ERGM.supports_missing ===
#    Networks.supports_missing`), so this line could equally be spelled
#    `ERGM.supports_missing(::ReciprocatedHomophily) = true`.
