...
@testset "MyTermPackage" begin
    ...
    @testset "Term is valid (interface + trait declarations)" begin
        ...
        # Seeded: the harness draws every random dyad from `rng`, so a red
        # run is the same run every time (and any failure message carries
        # the rng literal that replays it).
        @test validate_term(term, net; verbose=false, rng=Xoshiro(1))
    end
    ...
    @testset "ERGM model construction" begin
        ...
        # Always pass an rng to a fit (the ecosystem's randomness contract):
        # MPLE is deterministic, but the same call with method=:mcmle is not
        fit = fit_ergm(net, [Edges(), term]; rng=Xoshiro(1))
